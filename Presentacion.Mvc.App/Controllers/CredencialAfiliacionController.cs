...
{
    public class CredencialAfiliacionController : Controller
    {
        #region Field
...
#pragma warning restore CS0168 // The variable 'excepcion' is declared but never used
        ...
        [HttpPost]
        public ActionResult ReporteCredencialAfiliacion(ReportesModel viewModel)
        {
            try
            {
                ...
                {
                    if (viewModel.NumeroDoc != "" && viewModel.BeneficiarioSeleccionados.FirstOrDefault() == "-1")
                    {
                        return this.Report(
                            ReportFormat.PDF,
                            ConfiguracionesGlobales.ReportPathReporteCarnet,
                            ConfiguracionesGlobales.ReportesReportServerUrl,
                            new List<KeyValuePair<string, object>>
                            {
                                new KeyValuePair<string, object>("NumIdentificacion", viewModel.NumeroDoc), //contratante
                                new KeyValuePair<string, object>("Contrato", " ") //beneficiario
                            },
                            ConfiguracionesGlobales.ReportesUsername,
                            ConfiguracionesGlobales.ReportesPassword
                        );
                    }
                    else if (viewModel.NumeroDoc != "" && viewModel.BeneficiarioSeleccionados.FirstOrDefault() != "-1")
                    {
                        return this.Report(
                            ReportFormat.PDF,
                            ConfiguracionesGlobales.ReportPathReporteCarnet,
                            ...
                            {
                                new KeyValuePair<string, object>("NumIdentificacion",  viewModel.NumeroDoc), //contratante
                                new KeyValuePair<string, object>("Contrato", viewModel.BeneficiarioSeleccionados.FirstOrDefault()) //beneficiario
                            },
                            ConfiguracionesGlobales.ReportesUsername,
                            ConfiguracionesGlobales.ReportesPassword
                        );
