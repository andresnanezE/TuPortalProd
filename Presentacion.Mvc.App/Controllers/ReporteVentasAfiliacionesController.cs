...
#pragma warning disable CS0169 // The field 'ReporteVentasAfiliacionesController._servicioAplicacionTablasBase' is never used
        private IServicioAplicacionTablasBase _servicioAplicacionTablasBase;
#pragma warning restore CS0169 // The field 'ReporteVentasAfiliacionesController._servicioAplicacionTablasBase' is never used
        ...
        public ActionResult Index(string msg)
        {
            ...
            try
            {
                ...
                model.ListaFiltros = ServicioAplicacionAfiliaciones.ObtenerFiltroRoles(lstRoles);
...
#pragma warning restore CS0168 // The variable 'e' is declared but never used
        ...
        public ActionResult Index(AfiliacionesModel _modelRequest)
        {
            ...
            try
            {
                ...
                var roles = identity.FindFirst(ClaimTypes.Role).Value;
                var lstRoles = roles.TrimEnd(',').Split(',');
                _modelRequest.ListaFiltros = ServicioAplicacionAfiliaciones.ObtenerFiltroRoles(lstRoles);

                //_modelRequest.ListaFiltros = ServicioAplicacionAfiliaciones.ObtenerFiltro_x_Rol(int.Parse(usuarioTuPortal.Rol));
                string view = UserClaims.ValidarRolAsesor(lstRoles) ? "ResumenAsesor" : "ResumenDirector";
                ...
                ServicioAplicacionLogs.AgregarLog(log);
                if (!UserClaims.ValidarRolCadenaSupervision(lstRoles))
                {
                    var dto = new DatosAfiliacionDto();

                    dto.Documento = usuarioTuPortal.Document;
                    dto.Rol = roles.TrimEnd(',');
                ...
                if (UserClaims.ValidarRolCadenaSupervision(lstRoles))
                {
                    ...
                    _modelRequest.ListaResultadoResumenTabla =
                        ServicioAplicacionAfiliaciones.Consultar_Detalle_Afiliacion_ResumenTabla(new DatosConsultaAfiliacionDto()
                        {
                            ...
                            Rol = int.Parse(usuarioTuPortal.Rol.Replace(",", "")),
                            Periodo = DateTime.Parse(_modelRequest.Periodo),
                            Periodo2 =DateTime.Parse(_modelRequest.Periodo2),
                            Comisiona = _modelRequest.ListaFiltros.FirstOrDefault(x =>x.DatosFiltro.ID_CODIGO
                                        .Equals("Comisiona")).Mostar.Equals("N") ? _modelRequest.ListaFiltros
                                        .FirstOrDefault(x => x.DatosFiltro.ID_CODIGO
                                        .Equals("Comisiona")).FiltroDefault.Split(',').ToList() : _modelRequest.Comisiona,
                            EstBenef = _modelRequest.ListaFiltros.FirstOrDefault(x =>x.DatosFiltro.ID_CODIGO
                                        .Equals("EstBenef")).Mostar.Equals("N") ? _modelRequest.ListaFiltros
                                        .FirstOrDefault(x =>x.DatosFiltro.ID_CODIGO
                                        .Equals("EstBenef")).FiltroDefault.Split(',').ToList(): _modelRequest.EstBenef,
                            TipContrato =_modelRequest.ListaFiltros.FirstOrDefault(x =>x.DatosFiltro.ID_CODIGO
                                        .Equals("TipContrato")).Mostar.Equals("N") ? _modelRequest.ListaFiltros
                                        .FirstOrDefault(x => x.DatosFiltro.ID_CODIGO
                                        .Equals("TipContrato")).FiltroDefault.Split(',').ToList() : _modelRequest.TipContrato,
                            Novedad =   _modelRequest.ListaFiltros
                                        .FirstOrDefault(x => x.DatosFiltro.ID_CODIGO
                                        .Equals("Novedad")).Mostar.Equals("N") ? !string.IsNullOrWhiteSpace(_modelRequest.ListaFiltros
                                        .FirstOrDefault(x => x.DatosFiltro.ID_CODIGO.Equals("Novedad")).FiltroDefault) ? _modelRequest.ListaFiltros
                                        .FirstOrDefault(x => x.DatosFiltro.ID_CODIGO
                                        .Equals("Novedad")).FiltroDefault.Split(',')
                                        .ToList() : new List<string>(): _modelRequest.Novedad
                ...
                if (UserClaims.ValidarRolDirector(lstRoles))
                {
